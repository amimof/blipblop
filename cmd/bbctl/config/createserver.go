...
func NewCmdConfigCreateServer() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Add a server to bbctl client configuration",
		Long:  "Add a server to bbctl client configuration",
		Example: `
# Create a server with TLS
bbctl config create-server dev --address localhost:5743 --ca ca.crt
`,
