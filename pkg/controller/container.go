...
func (c *ContainerController) onContainerStart(ctx context.Context, obj *eventsv1.Event) error {
	...
	// _ = c.clientset.ContainerV1().Patch(ctx, ctr.GetMeta().GetName(), &containersv1.Container{Status: &containersv1.Status{Phase: containersv1.Phase_Stopping.String()}})
	...
	// _ = c.clientset.ContainerV1().Patch(ctx, ctr.GetMeta().GetName(), &containersv1.Container{Status: &containersv1.Status{Phase: containersv1.Phase_Pulling.String()}})
	err = c.runtime.Pull(ctx, &ctr)
	if err != nil {
		return err
	}
	...
	// _ = c.clientset.ContainerV1().Patch(ctx, ctr.GetMeta().GetName(), &containersv1.Container{Status: &containersv1.Status{Phase: containersv1.Phase_Starting.String()}})
	err = c.runtime.Run(ctx, &ctr)
	if err != nil {
		return err
	}
