...
func NodeVolumeManagers(n *nodes.Node) map[consts.VolumeType]volume.Driver {
	result := make(map[consts.VolumeType]volume.Driver)
	...
	if drivers.GetHostLocal() != nil {
		result[consts.VolumeTypeHostLocal] = volume.NewHostLocalManager(drivers.GetHostLocal().GetRootDir())
	}
