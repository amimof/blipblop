# TLS Certificates for Blipblop

This directory contains example certificate configuration and helper commands for running Blipblop components (server and nodes) over TLS.
...
- `server.conf` – OpenSSL configuration for generating the Blipblop server certificate and CSR.
- `server.key` – Private key for the Blipblop server (generated).
- `server.csr` – Certificate Signing Request for the Blipblop server (generated).
- `server.crt` – Signed certificate for the Blipblop server (generated).
...
Next, generate a private key and certificate for the Blipblop server. From the `certs/` directory, run:
